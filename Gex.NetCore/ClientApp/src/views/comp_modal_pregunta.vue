    ...
    <v-dialog :value="modal" fullscreen hide-overlay transition="dialog-bottom-transition">
      <v-card>
        ...
        <v-list three-line subheader>
          ...
          <v-list-item>
						<v-select :items="tipos" label="Tipo" v-model="pregunta.tipo"></v-select>
          </v-list-item>
        </v-list>
        <v-divider></v-divider>
        <div class="pa-4">
					...
					<div v-show="pregunta.tipo == 'Multiple Choice'">
						...
						<v-radio-group v-model="respuesta_correcta" class="ml-10">
...
<script>
	...
	export default {
		...
		data: () => ({
			...
			respuestas: [],
			nueva_respuesta_txt: '',
			tipos: ['Multiple Choice', 'Verdadero/Falso', 'Texto'],
